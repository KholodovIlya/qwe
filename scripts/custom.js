...
function startGame() {
  layers[0].context.font = "100px Monaco, monospace"; for(let i = 0; i < 3; i++) { layers.push(new Layer()); layers[1 + i].context.font = "100px Monaco, monospace"; }
  objects.push(new MoneyText());
  loadMenu();
}
...
class MoneyText extends GameObject {
  ...
  render() {
    clearTransform(this.transform, 3);
    renderImage(images[2], new Vector4(50, 150, 100, 100), 3);
...
class Background extends GameObject {
  ...
  render() {
    this.x -= this.speed; if(this.x <= 0) this.x = 1080;
    renderImage(images[3], new Vector4(this.x - 540, 960, 1080, 1920), 0);
    renderImage(images[3], new Vector4(this.x + 540, 960, 1080, 1920), 0);
  }
}
...
class Platform extends GameObject {
  constructor(y) { super(1180, y, 200, 1); this.speed = 10; }

  update() { if(isPaused) return; this.transform.position.x -= this.speed; if(this.transform.position.x < -100) this.destroyed = true; }
  lateUpdate() { if(!isPaused) this.render(); }
  render() { renderImage(images[1], new Vector4(this.transform.position.x, this.transform.position.y, 200, 100), 2); }
...
class ScoreText extends GameObject {
  ...
  render() {
    clearTransform(this.transform, 3);
    layers[3].context.fillText(float2int(this.score), 115, this.transform.position.y + 32);
  }
}
